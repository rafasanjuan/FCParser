...
export default () => {
  ...
  const runParser = () => {
    calculateTemplateYaml();
    const formData = new FormData();
    let sessionId = localStorage.getItem('session_id');
    formData.append("session_id", sessionId);
    formData.append("config[]", templateYaml);
    axios
  ...
  const downloadResults = () => {
    let sessionId = localStorage.getItem('session_id');
    window.open(`${apiUrl}/results/${sessionId}`);
    // axios
  ...
  const uploadYamlTemplate = (e) => {
    ...
    reader.addEventListener( "load", () => {
        ...
        calculateYamlGrid(reader.result);
  ...
  const calculateYamlGrid = (yamlInput) => {
    let config = jsyaml.load(yamlInput, 'utf8');
    if (config.hasOwnProperty('VARIABLES') && Array.isArray(config.VARIABLES)) {
    ...
    if (config.hasOwnProperty('FEATURES') && Array.isArray(config.FEATURES)) {
      ...
      >
        ...
        <EuiFlyoutFooter>
          <EuiFlexGroup justifyContent="spaceBetween">
            ...
            <EuiFlexItem grow={false}>
              <EuiButton onClick={() => { saveFreeEdit() }} fill>
                Save
              </EuiButton>
            </EuiFlexItem>
          </EuiFlexGroup>
