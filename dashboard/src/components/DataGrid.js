...
//   import { fake } from 'faker';
  ...
  for (let i = 1; i < 100; i++) {
    const email = 'correo@correo.com'; //fake('{{internet.email}}');
    const name =  'sanjuan, Rafael'; //fake('{{name.lastName}}, {{name.firstName}}');
    const suffix = 'Don.'; //fake('{{name.suffix}}');
    raw_data.push({
      name: {
      ...
      email: {
        formatted: <EuiLink href="">correo@correo.com</EuiLink>,
        raw: email,
      },
      location: (
        <Fragment>
      ...
      date: '2022-01-01', //fake('{{date.past}}'),
      account: 'Salesforce', // fake('{{finance.account}}'),
      amount: '2500$', //fake('${{commerce.price}}'),
      phone: '658580011',//fake('{{phone.phoneNumber}}'),
      version: 'v1.1',//fake('{{system.semver}}'),
    });
  }
